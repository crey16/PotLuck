 ...
 *
 * This page deliberately does NOT render the learning path. M8.5A briefly made
 * it do so and that was reverted: a first-run problem does not justify
 * restructuring the surface every established player sees on every visit.
 *
 * **Lesson-first is a routing property, not a layout one.** A new account is
 * sent from signup to `/` (see `app/login/page.tsx`), bounced from here into
 * `/placement`, and handed off to `/learn` — so it reaches this dashboard only
 * once it has something to show. `StartHereNudge` is the recovery path for
 * anyone that routing missed. If you are tempted to put the course map back on
 * this page, that is the thing to fix instead.
 */
export default async function Home() {
  ...
  // Placement runs once, before a brand-new account is shown anything else.
  // `needsPlacement` is true only with no assessment row AND no attempts, and
  // starting, completing or skipping all write a row — so this fires at most
  // once per account and can never trap an established player.
  ...
  return (
    <main className="page" style={{ paddingTop: "var(--space-8)" }}>
      <StartHereNudge routing={routing} dismissed={nudgeDismissed} />
